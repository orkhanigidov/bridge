...
namespace codegen {
    ...
    ClangAnalyzer::ClangAnalyzer(const std::string& filename,
        ...
        : m_target_classes(target_classes), m_target_free_functions(target_free_functions)
    {
        m_found_classes.clear();
        m_found_free_functions.clear();
        ...
        CXTranslationUnit translation_unit = clang_parseTranslationUnit(index, filename.c_str(), clang_args.data(),
                                                                        static_cast<int>(clang_args.size()), nullptr, 0, CXTranslationUnit_None);
        if (!translation_unit) {
    ...
    CXChildVisitResult ClangAnalyzer::visitor(CXCursor cursor, CXCursor parent, CXClientData client_data)
    {
        ...
        if (kind == CXCursor_ClassDecl && clang_isCursorDefinition(cursor)) {
            ...
            if (analyzer->m_target_classes.find(class_name) != analyzer->m_target_classes.end()) {
                ...
                clang_visitChildren(cursor, [](CXCursor child_cursor, CXCursor parent_cursor, CXClientData client_data) -> CXChildVisitResult {
                            auto* analyzer = static_cast<ClangAnalyzer*>(client_data);
                            if (!analyzer)
                            {
                                return CXChildVisit_Continue;
                            ...
                            if (child_kind == CXCursor_Constructor) {
                                ...
                                analyzer->m_found_classes.back().add_constructor(ctor_desc);
                            }
                                ...
                                analyzer->m_found_classes.back().add_base_class_name(base_class_name);
                            }
                                ...
                                if (std::find(target_methods.begin(), target_methods.end(), method_name) != target_methods.end()) {
                                    ...
                                    analyzer->m_found_classes.back().add_member_function(method_desc);
                                }
                            }
                                ...
                                if (clang_getCXXAccessSpecifier(child_cursor) == CX_CXXPublic) {
                                    ...
                                    analyzer->m_found_classes.back().add_member_variable(var_desc);
                                }
                            }
                            return CXChildVisit_Continue;
                        }, client_data);

                analyzer->m_found_includes.emplace_back(get_include_path(cursor));
            }
        }
            ...
            if (std::find(analyzer->m_target_free_functions.begin(), analyzer->m_target_free_functions.end(), func_name) != analyzer->m_target_free_functions.end()) {
                ...
                analyzer->m_found_free_functions.emplace_back(func_desc);
                analyzer->m_found_includes.emplace_back(get_include_path(cursor));
            }
        }
