...
namespace codegen::generation
{
                                 ...
                                 const std::vector<std::pair<std::string, std::string>>& manual_bindings) const
    {
        ...
        write_header(out, includes, namespaces, containers, classes, manual_bindings);
        write_member_registrations(out, classes);
        write_non_member_registrations(out, free_functions, enums);
        write_manual_bindings(out, manual_bindings);
        write_footer(out);
    }
                                     ...
                                     const std::vector<std::pair<std::string, std::string>>& manual_bindings)
    {
        ...
        write_line(out, 0, "#include \"bindings/lua/utils/sol_trait_macros.hpp\"", 2);
        ...
        if (!manual_bindings.empty())
        {
            write_line(out, 0, "");
            write_line(out, 0, "// Manual binding headers");

            for (const auto& binding : manual_bindings)
            {
                std::string include_path = binding.first;
                std::replace(include_path.begin(), include_path.end(), '\\', '/');
                write_line(out, 0, "#include \"" + include_path + "\"");
            }
        }
        ...
        {
            write_line(out, 1, "DISABLE_SOL_AUTOMAGICAL(" + cls.name() + ");");
        }
        ...
        for (const auto& cls : classes)
        {
            for (const auto& var : cls.member_variables())
            {
                if (var.is_container())
                {
                    all_containers.emplace(var.type_name());
                }
            }
        }
        ...
        {
            write_line(out, 1, "DISABLE_SOL_CONTAINER(" + container + ");");
            write_line(out, 1, "DISABLE_SOL_AUTOMAGICAL(" + container + ");");
        }
        write_line(out, 0, "}", 2);
    ...
    void Sol2Generator::write_manual_bindings(std::ofstream& out, const std::vector<std::pair<std::string, std::string>>& manual_bindings)
    {
        write_line(out, 1, "void register_manual_bindings(sol::state& lua)");
        write_line(out, 1, "{");

        if (manual_bindings.empty())
        {
            write_line(out, 2, "(void)lua; // No manual bindings");
        }
        else
        {
            for (const auto& binding : manual_bindings)
            {
                write_line(out, 2, binding.second + "(lua);");
            }
        }
