...
namespace codegen {
    ClangAnalyzer::ClangAnalyzer(const std::string& filename, const std::unordered_map<std::string, std::vector<std::string>>& target_classes, const std::vector<std::string>& target_free_functions)
        : target_classes_(target_classes), target_free_functions_(target_free_functions)
    {
        ...
        for (const auto& arg: clang_args_str) { clang_args.emplace_back(arg.c_str()); }

        CXTranslationUnit translation_unit = clang_parseTranslationUnit(index, filename.c_str(), clang_args.data(), static_cast<int>(clang_args.size()), nullptr, 0, CXTranslationUnit_None);
        if (!translation_unit) {
    ...
    CXChildVisitResult ClangAnalyzer::visitor(CXCursor cursor, CXCursor parent, CXClientData client_data)
    {
        ...
        if (!analyzer) { return CXChildVisit_Continue; }
        ...
        if (kind == CXCursor_ClassDecl && clang_isCursorDefinition(cursor)) {
                ...
                clang_visitChildren(
                        ...
                        [](CXCursor child_cursor, CXCursor parent_cursor, CXClientData client_data) -> CXChildVisitResult {
                            ...
                            if (child_kind == CXCursor_Constructor) {
                                ...
                                engine::meta::ConstructorDescriptor ctor_desc;
                                ctor_desc.set_name(ctor_name);
                                ...
                                for (size_t i = 0; i < params.size(); ++i) {
                                    ...
                                    if (i != params.size() - 1) { overload_signature += ", "; }
                                }
                                ...
                                if (std::find(target_methods.begin(), target_methods.end(), method_name) != target_methods.end()) {
                                    ...
                                    method_desc.set_return_type(get_result_type(child_cursor));
                                    if (clang_CXXMethod_isStatic(child_cursor)) { method_desc.set_static(true); }
                                    if (clang_CXXMethod_isConst(child_cursor)) { method_desc.set_const(true); }
                                    ...
                                    analyzer->found_classes_.back().add_method(method_desc);
                                }
                            }
                                ...
                                if (clang_getCXXAccessSpecifier(child_cursor) == CX_CXXPublic) {
                                    ...
                                    if (clang_Cursor_getStorageClass(child_cursor) == CX_SC_Static) { var_desc.set_static(true); }
                                    analyzer->found_classes_.back().add_variable(var_desc);
                                }
                            }
            ...
            if (std::find(analyzer->target_free_functions_.begin(), analyzer->target_free_functions_.end(), func_name) != analyzer->target_free_functions_.end()) {
                engine::meta::FunctionDescriptor func_desc;
                func_desc.set_name(func_name);
                func_desc.set_return_type(get_result_type(cursor));
    ...
    std::string ClangAnalyzer::get_spelling(const CXCursor& cursor)
    {
    ...
    std::string ClangAnalyzer::get_result_type(const CXCursor& cursor)
    {
    ...
    std::string ClangAnalyzer::get_type(const CXCursor& cursor)
    {
    ...
    std::string ClangAnalyzer::get_qualified_type(const CXCursor& cursor)
    {
        const CXType type = clang_getCursorType(cursor);
        ...
        if (!clang_Cursor_isNull(type_decl)) {
            ...
            if (!parent_name.empty()) { return parent_name + "::" + type_name; }
        }
        return type_name;
    }

    std::vector<engine::meta::ParameterDescriptor> ClangAnalyzer::get_arguments(const CXCursor& cursor)
    {
        ...
        for (int i = 0; i < num_args; ++i) {
            ...
            param_desc.set_type(param_type);
